            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {order.createdAt && (
                <span
                  ...
                  className="inline-flex items-center gap-1 text-[11px] font-bold text-slate-600 bg-slate-100 px-2.5 py-0.5 rounded-full border border-slate-200/70"
        ...
        {order.createdAt && (
          <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-100 space-y-2 text-xs">
            <div className="flex items-center justify-between text-slate-600 flex-wrap gap-1">
              ...
              <span suppressHydrationWarning className="font-bold text-slate-900">
                {formatOrderDateTime(order.createdAt)}
              </span>
            </div>
            {order.rider_accepted_at && (
              <div className="flex items-center justify-between text-slate-600 flex-wrap gap-1 pt-1.5 border-t border-slate-200/60 text-[11px]">
                <span className="flex items-center gap-1.5 font-medium text-slate-600">
                  <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500 shrink-0" />
                  Claimed by You:
                </span>
                <span suppressHydrationWarning className="font-semibold text-slate-800">
                  {formatOrderTime(order.rider_accepted_at)}
                </span>
              </div>
            )}
            {order.food_ready_at && (
              <div className="flex items-center justify-between text-slate-600 flex-wrap gap-1 pt-1.5 border-t border-slate-200/60 text-[11px]">
                ...
                <span suppressHydrationWarning className="font-semibold text-slate-800">
                  {formatOrderTime(order.food_ready_at)}
                </span>
              </div>
            )}
            {order.picked_up_at && (
              <div className="flex items-center justify-between text-slate-600 flex-wrap gap-1 pt-1.5 border-t border-slate-200/60 text-[11px]">
                ...
                <span suppressHydrationWarning className="font-semibold text-slate-800">
                  {formatOrderTime(order.picked_up_at)}
                </span>
              </div>
            )}
          </div>
