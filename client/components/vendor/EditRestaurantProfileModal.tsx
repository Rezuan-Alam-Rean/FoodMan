      ...
      <div className="w-full sm:max-w-lg bg-white rounded-t-[32px] sm:rounded-3xl p-5 sm:p-6 pb-[calc(1.5rem+env(safe-area-inset-bottom,0px))] sm:pb-6 shadow-2xl border-t sm:border border-slate-200 max-h-[92vh] sm:max-h-[90vh] flex flex-col overflow-hidden animate-in slide-in-from-bottom-8 sm:slide-in-from-bottom-0 sm:zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between border-b border-slate-100 pb-3 shrink-0">
          ...
          <button
            type="button"
            onClick={onClose}
            className="w-11 h-11 rounded-2xl text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition flex items-center justify-center cursor-pointer shrink-0 active:scale-95"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        >
          ...
          <div className="space-y-1.5">
            ...
            {isCoverManual ? (
              <input
                ...
                className="w-full min-h-[44px] sm:min-h-[40px] px-3.5 py-2.5 rounded-2xl bg-slate-50 border border-slate-200 text-slate-900 text-base sm:text-xs font-mono focus:outline-none focus:ring-2 focus:ring-rose-500/20 placeholder:font-sans placeholder:text-slate-400"
              />
            ) : coverImageUrl ? (
              <div className="relative rounded-2xl border border-slate-200 overflow-hidden bg-slate-100 h-28 w-full group">
                ...
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                  <button
                    ...
                    className="min-h-[38px] px-3.5 py-2 rounded-xl bg-white text-slate-800 text-xs font-bold hover:bg-slate-50 transition cursor-pointer flex items-center gap-1 active:scale-95"
                  >
                  ...
                  <button
                    ...
                    className="min-h-[38px] min-w-[38px] p-2 rounded-xl bg-rose-600 text-white hover:bg-rose-700 transition cursor-pointer flex items-center justify-center active:scale-95"
              ...
              <button
                ...
                onClick={() => coverInputRef.current?.click()}
                disabled={isUploadingCover}
                className="w-full h-24 rounded-2xl border-2 border-dashed border-slate-200 hover:border-slate-300 bg-slate-50 hover:bg-slate-100/70 transition flex flex-col items-center justify-center gap-1.5 cursor-pointer disabled:opacity-50 active:scale-[0.99]"
              >
                {isUploadingCover ? (
          ...
          <div className="space-y-1.5">
            ...
            {isLogoManual ? (
              ...
              <div className="flex items-center gap-3 p-3 rounded-2xl bg-slate-50 border border-slate-200">
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                  <button
                    ...
                    className="min-h-[38px] px-3 py-2 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 text-xs font-bold transition cursor-pointer active:scale-95"
                  >
                    Change
                  </button>
                  <button
                    type="button"
                    onClick={() => setLogoUrl('')}
                    className="min-h-[38px] min-w-[38px] p-2 rounded-xl border border-rose-100 hover:bg-rose-50 text-rose-500 transition cursor-pointer flex items-center justify-center active:scale-95"
                    title="Remove logo"
                  >
                    <Trash2 className="w-3.5 h-3.5" />
                  </button>
              ...
              <button
                ...
                onClick={() => logoInputRef.current?.click()}
                disabled={isUploadingLogo}
                className="w-full p-4 rounded-2xl border-2 border-dashed border-slate-200 hover:border-slate-300 bg-slate-50 hover:bg-slate-100/70 transition flex items-center justify-center gap-3 cursor-pointer disabled:opacity-50 active:scale-[0.99]"
              >
                {isUploadingLogo ? (
                  <div className="flex items-center gap-2 text-xs font-bold text-slate-600">
                    <Loader2 className="w-4 h-4 text-rose-600 animate-spin" />
          ...
          <div className="space-y-1">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full min-h-[44px] sm:min-h-[40px] pl-10 pr-3.5 py-2.5 rounded-2xl bg-slate-50 border border-slate-200 text-slate-900 text-base sm:text-xs font-bold focus:outline-hidden focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500"
              />
            </div>
          </div>
          ...
          <div className="space-y-1 relative" ref={zoneDropdownRef}>
            ...
            <button
              ...
              className={`w-full min-h-[44px] sm:min-h-[40px] px-3.5 py-2.5 rounded-2xl bg-slate-50 border text-left text-base sm:text-xs font-bold transition flex items-center justify-between gap-2 cursor-pointer ${
                isZoneOpen
                ...
                {zones.map((zone) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full min-h-[40px] px-3 py-2.5 rounded-xl text-left text-sm sm:text-xs font-bold transition flex items-center justify-between gap-2 cursor-pointer ${
                        isSelected
        ...
        <div className="pt-3 border-t border-slate-100 flex items-center gap-3 shrink-0">
          <button
            type="button"
            onClick={onClose}
            className="flex-1 min-h-[48px] py-3 rounded-2xl border border-slate-200 text-slate-700 text-sm sm:text-xs font-black hover:bg-slate-50 active:scale-[0.98] transition cursor-pointer flex items-center justify-center"
          >
            Cancel
          </button>
          <button
            ...
            className="flex-1 min-h-[48px] py-3 rounded-2xl bg-rose-600 text-white text-sm sm:text-xs font-black hover:bg-rose-700 active:scale-[0.98] transition shadow-md shadow-rose-600/20 flex items-center justify-center gap-1.5 disabled:opacity-50 cursor-pointer"
          >
            {isSaving ? (
